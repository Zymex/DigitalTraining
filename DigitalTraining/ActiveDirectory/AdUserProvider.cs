...
using DigitalTraining.ActiveDirectory.Extensions;
...
namespace DigitalTraining.ActiveDirectory
{
    public class AdUserProvider : IUserProvider
    {
        ...
        public Task<List<AdUser>> GetDomainUsers()
        {
            ...
            {
                PrincipalContext context = new PrincipalContext(ContextType.Domain);
                UserPrincipal principal = new UserPrincipal(context) {UserPrincipalName = "*@*", Enabled = true};
        ...
        public Task<List<AdUser>> FindDomainUser(string search)
        {
            ...
            {
                PrincipalContext context = new PrincipalContext(ContextType.Domain);
                UserPrincipal principal = new UserPrincipal(context) {SamAccountName = $"*{search}*", Enabled = true};
                PrincipalSearcher searcher = new PrincipalSearcher(principal);
